...
export default class Setting extends React.Component{
  generateImageInfo(imageInfo){
    return imageInfo.map((item,index) =>{
      if(item.keyword != ""){
        return <RadioButton key ={index} value = {index}><Tooltip placement="top" title={item.engine}><a href ={item.keywordLink} target ="_blank">{item.keyword}</a></Tooltip></RadioButton>
      }
    })
  }
  render(){
